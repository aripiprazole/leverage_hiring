...
async fn create_publishes_complete_vm_and_discovery_preserves_its_id() {
    ...
    let created = store
        ...
        .await
        .unwrap();
    assert_eq!(u16::from(created.vm_id), 0);
...
async fn create_uses_default_keys_and_reuses_only_deleted_ids() {
    ...
    assert_eq!(u16::from(alice.vm_id), 0);
    assert_eq!(u16::from(bob.vm_id), 1);
    assert_eq!(
    ...
    assert_eq!(u16::from(carol.vm_id), 0);
}
...
async fn discovery_rejects_symlinked_vm_directories_and_duplicate_ids() {
    ...
    config["spec"]["vm_id"] = serde_json::json!(u16::from(alice.vm_id));
    config["spec"]["network"]["vm_id"] = serde_json::json!(u16::from(alice.vm_id));
